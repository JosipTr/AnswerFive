...
class PickerBloc extends Bloc<PickerEvent, PickerState> {
  ...
  final GetCameraPicture _getCameraPicture;

  PickerBloc(this._getGalleryPicture, this._getCameraPicture)
      : super(const PickerInitial()) {
    on<PickerImagePicked>(_onPickerImagePicked);
    on<PickerCameraPicked>(_onPickerCameraPicked);
  }
  ...
  Future<void> _onPickerCameraPicked(
      PickerCameraPicked event, Emitter<PickerState> emit) async {
    final either = await _getCameraPicture(const NoParams());
